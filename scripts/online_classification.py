# online_audio_classification_load_pkl.py
import time
import requests
import numpy as np
import threading
...
import pickle # Asegúrate de que pickle esté importado
            ...
            if time_buffer_from_phyphox and db_buffer_from_phyphox:
                ...
                with buffer_lock:
                    if not buffer_data or buffer_data[-1][0] < current_time_float:
                        ...
                        max_buffer_len = int(max_samp_rate_phyphox * (window_time * 4)) 
...
prediction_count = 0
min_raw_points_needed_for_interp = window_samples * 2 # Mínimo de puntos crudos para interpolar
...
try:
    while True:
        ...
        snapshot_buffer_data_main.sort(key=lambda x: x[0]) 
